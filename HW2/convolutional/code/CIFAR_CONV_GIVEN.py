...
tf.app.flags.DEFINE_integer('total_epochsG', 50, """Number of Total Epochs""")
tf.app.flags.DEFINE_float('learning_rateG', 0.0001, """Learning Rate""")
...
def inference(images):
    ...
    with tf.name_scope('conv1'):
        ...
        kernel1 = tf.get_variable('kernel1', shape=[5, 5, 3, 32])
        c1=tf.nn.conv2d(images, kernel1, [1,1,1,1], padding='SAME')
        b1=tf.get_variable('bias1', shape=[32], initializer=tf.zeros_initializer())
    ...
    with tf.name_scope('conv2'):
        #conv+bias
        kernel2 = tf.get_variable('kernel2', shape=[5, 5, 32, 32])
        c2=tf.nn.conv2d(pool1, kernel2, [1,1,1,1], padding='SAME')
        b2=tf.get_variable('bias2', shape=[32], initializer=tf.zeros_initializer())
        ...
        print(conv2)
        #pool
        pool2=tf.nn.max_pool(conv2, [1,2,2,1], strides=[1,2,2,1], padding='SAME')
        print(pool2)
...
def trainConvGivenModel(data):
    tf.reset_default_graph()
    import time
    print ("Training with Given Conv model")
    ...
    for epoch in range(FLAGS.total_epochsG):
        ...
        if (epoch+1)%10==0 or (epoch+1)==FLAGS.total_epochsG:
            checkpoint_file=os.path.join(FLAGS.log_dirG, 'GConvmodel.ckpt')
            saver.save(sess, checkpoint_file, global_step=epoch)
            print('TEST Data Eval(Given): ')
            true_count=0
            for start_idx in range(0, len(test_images), FLAGS.batch_sizeG):
                batch_images=test_images[start_idx:start_idx+FLAGS.batch_sizeG]
                batch_labels=test_labels[start_idx:start_idx+FLAGS.batch_sizeG]
                test_feed = {images_placeholder : np.reshape(batch_images, (-1, 32, 32, 3)),
                             labels_placeholder : np.reshape(batch_labels, (-1, 1))}
                true_count+=(FLAGS.batch_sizeG*sess.run(accuracy, feed_dict=test_feed))
    ...
    print("Result of Given Conv Model with Test Set: ", precision)
    import matplotlib.pyplot as plt
    plt.figure(1)
    plt.title('loss history (Given Conv Model)')
