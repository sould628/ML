...
tf.app.flags.DEFINE_integer('total_epochsM', 100, """Number of Total Epochs""")
tf.app.flags.DEFINE_integer('hidden1M', 2048, """Number of 1st hidden units""")
tf.app.flags.DEFINE_integer('hidden2M', 1024, """Number of 2nd hidden units""")
tf.app.flags.DEFINE_integer('hidden3M', 512, """Number of 3rd hidden units""")
tf.app.flags.DEFINE_integer('hidden4M', 128, """Number of 4th hidden units""")
tf.app.flags.DEFINE_float('learning_rateM', 0.0001, """Learning Rate""")
tf.app.flags.DEFINE_string('log_dirM', './log/','''Path to the log''')
FLAGS=tf.app.flags.FLAGS

def inference(images, hidden1_units, hidden2_units, hidden3_units, hidden4_units):
    ...
    with tf.name_scope('hidden4'):
        w4=tf.get_variable(
            'w4',
            shape=[hidden3_units, hidden4_units],
            initializer=tf.contrib.layers.xavier_initializer()
        )
        b4=tf.get_variable('b4', shape=[hidden4_units], initializer=tf.zeros_initializer())
        hidden4=tf.nn.tanh(tf.matmul(hidden3, w4)+b4)

    with tf.name_scope('softmax_linear'):
        wo=tf.get_variable(
            'wo',
            shape=[hidden4_units, NUM_CLASSES],
            initializer=tf.contrib.layers.xavier_initializer()
        )
...
def trainFFMyModel(data):
    tf.reset_default_graph()
    import time
    print ("Training My FF model")
    ...
    output_layer = inference(images_placeholder, FLAGS.hidden1M, FLAGS.hidden2M, FLAGS.hidden3M, FLAGS.hidden4M)
    ...
    for epoch in range(FLAGS.total_epochsM):
        ...
        if (epoch+1)%10==0 or (epoch+1)==FLAGS.total_epochsM:
            checkpoint_file=os.path.join(FLAGS.log_dirM, 'MFFmodel.ckpt')
            saver.save(sess, checkpoint_file, global_step=epoch)
            print('Training Data Eval(My FF):')
            do_eval(sess, accuracy, images, labels, images_placeholder, labels_placeholder, NUM_EXAMPLES_PER_EPOCH_FOR_TRAIN, FLAGS.batch_sizeM)

    import matplotlib.pyplot as plt
    plt.figure(1)
    plt.title('loss history (My FF Model)')
    ...
    print("Result of My FF Model with Test Set: ", test_acc)
